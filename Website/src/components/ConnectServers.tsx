...
export function ConnectServers() {
  ...
  const { focusElement } = useTVNavigation();
  ...
  const focusByGroupOrder = (currentOrder: number, direction: 'up' | 'down') => {
    const group = 'connect-flow';
    const elements = Array.from(
      document.querySelectorAll<HTMLElement>(`[data-tv-group="${group}"][data-tv-order]`)
    ).sort((a, b) => Number(a.dataset.tvOrder) - Number(b.dataset.tvOrder));

    const currentIndex = elements.findIndex((el) => Number(el.dataset.tvOrder) === currentOrder);
              ...
              <select
                ...
                onKeyDown={(e) => {
                  if (e.key === 'ArrowDown') {
                    e.preventDefault();
                    e.stopPropagation();
                    focusByGroupOrder(1, 'down');
                  } else if (e.key === 'ArrowUp') {
                    e.preventDefault();
                    e.stopPropagation();
                    focusByGroupOrder(1, 'up');
                  }
                }}
